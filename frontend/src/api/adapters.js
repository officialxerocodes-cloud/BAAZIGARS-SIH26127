...
// ---- Feed liveness -----------------------------------------------------
// One honest vocabulary for every widget: 'live' (backend answering fresh),
// 'stale' (backend answered before but the last poll failed or data is old),
// 'offline' (backend never answered — demo/fixture data is on screen).
// Callers gate ALL mock fallbacks on this: empty-but-live shows an honest
// empty state, never mock rows disguised as live data.
export function feedState({ updatedAt, error, pollMs = 15000 } = {}) {
  if (updatedAt == null) return "offline";
  const staleAfter = Math.max(45000, (pollMs || 15000) * 2.5);
  if (error != null || Date.now() - updatedAt > staleAfter) return "stale";
  return "live";
}
...
export function buildDensityProps(
  ...
  baseHours,
  baseChart,
  { congestion, heatmap, live = false } = {}
) {
  const out = {
    points: live ? [] : basePoints,
    hours: live ? [] : baseHours,
    chart: live
      ? {
          ...baseChart,
          labels: [],
          data: [],
          totalDaily: "No data",
          peakLabel: "No live data in window",
          peakTooltip: "No volume data in this window — run the sim feed",
          empty: true,
        }
      : baseChart,
  };
